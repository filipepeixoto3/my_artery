...
{    
    ...
    auto cpm = createCollectivePerceptionMessage(
        *mVehicleDataProvider, genDeltaTimeMod, *mVehicleController, uss_setups, detections_map, *detections, cur_pos_center);
    objects.clear();
    ...
    omnetpp::cGroupFigure& det, omnetpp::cFigure::Point cpc)
{
    ...
    for (auto pair : tracks_map) {
        ...
        uss_setup desired_config = uss_setups[value.sensor];
    ...
    perceivedObjects = (long)tracks_map.size();
    ...
    for (auto pair : tracks_map) {
        ...
        po->objectID = key;
        po->timeOfMeasurement = (long)(genDeltaTime - value.timestamp); 
...
void CpService::updateEveryTimestamp()
{
    ...
    for (auto pair : tracks_map) {
        key = pair.first
        value = pair.second
        if (timestamp - value.timestamp > 5 && std::find(active_tracks.begin(), active_tracks.end(), key) != active_tracks.end()) {
...
void CpService::storeDetections(int role_name, double timestamp, uint16_t detection_delta_time, double depth)
{
    ...
    if(role_name != 6 && detections_map.find(role_name - 1) != detections_map.end())
        if (depth < detections_map[role_name - 1].depth) 
            detections_map.erase(role_name - 1)

void CpService::multiObjectTracking(){
    if (active_tracks.empty()) {
        for (const auto& dm : detections_map) {
            key = dm.first;
            value = dm.second;
            //tracks_map[tracks_count_id] = std::make_tuple(value.detection_coord.x, value.detection_coord.y, value.timestamp);
            tracks_map[tracks_count_id].detection_coord = value.detection_coord;
            tracks_map[tracks_count_id].timestamp = timestamp;
            tracks_map[tracks_count_id].sensor = key;
        ...
        auto cost_matrix = generate_cost_matrix();
        ...
        if (active_tracks.size() == detections_map.size()) {
            ...
            for (size_t i = 0; i < active_tracks.size(); ++i) {
                // tracks_map[active_tracks[i]] = std::make_tuple(pos[min_cost_indices[i]][0], pos[min_cost_indices[i]][1], timestamp);
                // tracks_map[active_tracks[i]] = std::make_tuple(detections_map[keys[min_cost_indices[i]]].detection_coord.x ,detections_map[keys[min_cost_indices[i]]].detection_coord.y , detections_map[keys[pos_aux]].timestamp);
                tracks_map[active_tracks[i]].detection_coord = detections_map[keys[min_cost_indices[i]]].detection_coord;
                tracks_map[active_tracks[i]].timestamp = detections_map[keys[min_cost_indices[i]]].timestamp;
            ...
            for (size_t i = 0; i < detections_map.size() - active_tracks.size(); ++i) {
                
                //tracks_map[tracks_count_id] = std::make_tuple(pos[active_tracks.size() - 1 + i][0], pos[active_tracks.size() - 1 + i][1], timestamp);
                //tracks_map[tracks_count_id] = std::make_tuple(detections_map[keys[active_tracks.size() - 1 + i]].detection_coord.x ,detections_map[keys[active_tracks.size() - 1 + i]].detection_coord.y , detections_map[keys[pos_aux]].timestamp);
                tracks_map[tracks_count_id].detection_coord = detections_map[keys[active_tracks.size() - 1 + i]].detection_coord;
                tracks_map[tracks_count_id].timestamp = detections_map[keys[active_tracks.size() - 1 + i]].timestamp;
                tracks_map[tracks_count_id].sensor = keys[keys[active_tracks.size() - 1 + i]];            
            ...
            for (size_t j = 0; j < detections_map.size(); ++j) {
                ...
                // tracks_map[active_tracks[tracks_aux]] = std::make_tuple(detections_map[keys[pos_aux]].detection_coord.x, detections_map[keys[pos_aux]].detection_coord.y, detections_map[keys[pos_aux]].timestamp);
                tracks_map[active_tracks[tracks_aux]].detection_coord = detections_map[keys[pos_aux]].detection_coord;
                tracks_map[active_tracks[tracks_aux]].timestamp = detections_map[keys[pos_aux]].timestamp;
    ...
    for (const auto& at : active_tracks) {
        ...
        kf_map[at].update({std::get<0>(tracks_map[at]), std::get<1>(tracks_map[at])});
    }
...
//         if(key != 5 && key != 11 && detections_map.find(key + 1) != detections_map.end()){
//             if (value.depth < detections_map[key + 1].depth) {
//                 object_info oi;
//                 oi.objectID = 0;
//                 oi.timestamp = value.timestamp;
//                 oi.detection_delta_time = value.detection_delta_time;
//                 oi.depth = value.depth;
//                 oi.detection_coord = value.detection_coord;
//                 oi.xDistance = (value.detection_coord.x - cur_pos_center.x) * 100;
//                 oi.yDistance = (value.detection_coord.y - cur_pos_center.y) * 100;
//                 oi.sensor = key;
...
// void CpService::filterDetections()
// {
//     for (auto it = detections_map.begin(); it != detections_map.end(); ++it) {
//         auto key = it->first;
//         auto value = it->second;
//         if (key != 5 && key != 11 && detections_map.find(key + 1) != detections_map.end()) {
//             if (value.depth < detections_map[key + 1].depth) {
//                     detections_map.erase(key + 1);
//                 } else{
//                     detections_map.erase(key);
//                 }
//         }
//     }
...
//     for (auto it = detections_map.begin(); it != detections_map.end(); ++it) {
//         auto value = it->second;
//         omnetpp::cFigure::Point final_pos = {value.detection_coord.x, value.detection_coord.y};
//         omnetpp::cFigure::Point a = omnetpp::cFigure::Point(final_pos.x + 0.1, final_pos.y + 0.1);
//         omnetpp::cFigure::Point b = omnetpp::cFigure::Point(final_pos.x + 0.1, final_pos.y - 0.1);
//         omnetpp::cFigure::Point c = omnetpp::cFigure::Point(final_pos.x - 0.1, final_pos.y - 0.1);
//         omnetpp::cFigure::Point d = omnetpp::cFigure::Point(final_pos.x - 0.1, final_pos.y + 0.1);
...
//     // detections_map[role_name].last_detection_coord = detections_map[role_name].detection_coord;
//     detections_map[role_name].detection_coord = final_pos;
...
//         //} else if (detections_map[role_name].detection_coord.x != coord.x || detections_map[role_name].detection_coord.y != coord.y ) {
...
//         auto dist = std::round(
//                         std::sqrt(
//                             std::pow((detections_map[role_name].detection_coord.x - coord.x), 2) +
//                             std::pow((detections_map[role_name].detection_coord.y - coord.y), 2)) *
//                         1000) /
//                     1000;
...
//         //    << "detections_map[role_name].detection_coord.x: " << detections_map[role_name].detection_coord.x << endl
//         //    << "detections_map[role_name].detection_coord.y: " << detections_map[role_name].detection_coord.y << endl
...
//         omnetpp::cFigure::Point pq = coord - detections_map[role_name].detection_coord;
...
//             static_object[role_name] = detections_map[role_name].detection_coord;
//         }
//     }
//     fullPathToFile();
// }
...
//                 //     detections_map[key + 1].depth, value.detection_coord,
//                 //     detections_map[key + 1].detection_coord);  // triangulation
//                 //      oi.detection_coord = possible_position;

//                 if (value.depth < detections_map[key + 1].depth) {
//                     oi.detection_coord = value.detection_coord;
//                 } else {
//                     oi.detection_coord = detections_map[key + 1].detection_coord;
//                 }
...
//                 if ((oi.detection_coord.x != value.detection_coord.x || oi.detection_coord.y != value.detection_coord.y) &&
//                     (oi.detection_coord.x != detections_map[key + 1].detection_coord.x ||
//                      oi.detection_coord.y != detections_map[key + 1].detection_coord.y)) {

//                     // EV << oi.detection_coord.x << " != " << value.detection_coord.x << endl;
//                     // EV << oi.detection_coord.y << " != " << value.detection_coord.y << endl;
//                     // EV << oi.detection_coord.x << " != " << detections_map[key + 1].detection_coord.x << endl;
//                     // EV << oi.detection_coord.y << " != " << detections_map[key + 1].detection_coord.y << endl;

//                     omnetpp::cFigure::Point pq = value.detection_coord - value.last_detection_coord;
//                     double magPq = std::sqrt(std::pow(pq.x, 2) + std::pow(pq.y, 2));
//                     double angl = std::acos(pq.x / magPq);
//                     if (pq.y > 0)
//                         angl = angl + PI;
...
//                     // distance between the previous coord and the present one
//                     auto dist = std::sqrt(
//                         std::pow((value.last_detection_coord.x - oi.detection_coord.x), 2) +
//                         std::pow((value.last_detection_coord.y - oi.detection_coord.y), 2));
...
//                     oi.xDistance = (oi.detection_coord.x - cur_pos_center.x) * 100;
//                     oi.yDistance = (oi.detection_coord.y - cur_pos_center.y) * 100;
//                     oi.depth = value.depth;
...
//                 // const traci::Boundary boundary{mVehicleController->getTraCI()->simulation.getNetBoundary()};
//                 // artery::Position arteryPos = Position(value.detection_coord.x, value.detection_coord.y);
...
std::vector<std::vector<double>> CpService::generate_cost_matrix(
    const std::map<int, std::tuple<double, double, double>>& tracks_map,
    const std::vector<int>& active_tracks,
    const std::vector<int>& past_active_tracks,
    const std::map<int, std::tuple<double, double, double>>& past_tracks_map,
    double distance_weight = 0.3,
    double direction_weight = 1.0) {

    size_t n = active_tracks.size();
    size_t m = detections_map.size();
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        for (auto pair : detections_map) {
            ...
            auto pred_track_pos = tracks_map.at(active_tracks[i]);
            auto detection_pos = value.detection_coord;
            auto past_tracks_map_pos = past_tracks_map.at(past_active_tracks[i]);

            double dx = std::get<0>(pred_track_pos) - detection_pos.x;
            double dy = std::get<1>(pred_track_pos) - detection_pos.y;

            double distance = std::sqrt(dx * dx + dy * dy);
            double ang_dif = 0.0;

            if (!past_active_tracks.empty()) {
                double delta_x = std::get<0>(past_tracks_map_pos) - std::get<0>(pred_track_pos);
                double delta_y = std::get<1>(past_tracks_map_pos) - std::get<1>(pred_track_pos);

                double angle1 = std::atan2(delta_y, delta_x);

                delta_x = std::get<0>(pred_track_pos) - detection_pos.x;
                delta_y = std::get<1>(pred_track_pos) - detection_pos.y;
