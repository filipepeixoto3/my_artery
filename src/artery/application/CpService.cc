...
//     if (T_elapsed >= T_GenCpmDcc && T_now >= 1 && active_tracks.size() != 0 && simTime() >= 61) {
//         if (mFixedRate) {
//             sendCpm(T_now);
...
//     if(mVehicleController->getVehicleId() == "car0"){
//         for (auto key : active_tracks) {
//             auto value = tracks_map[key];
...
//         *mVehicleDataProvider, genDeltaTimeMod, *mVehicleController, uss_setups, detections_map, tracks_map, *detections, cur_pos_center, active_tracks);
...
//     omnetpp::cGroupFigure& det, omnetpp::cFigure::Point cpc, std::vector<int> at)
// {
...
//         if (timestamp - it->second.timestamp > 1 && std::find(active_tracks.begin(), active_tracks.end(), it->first) != active_tracks.end() || 
//         (timestamp - it->second.timestamp > 0.5 && detections_map[tracks_map[it->first].sensor].depth >=5)) {
            
//             // cLineFigure* line1 = tracks_map[it->first].cross_line1;
//             // cLineFigure* line2 = tracks_map[it->first].cross_line2;
//             // if (line1 != nullptr && line2 != nullptr) {
//             //     detections->removeFigure(line1);
//             //     detections->removeFigure(line2);
//             // }
//             active_tracks.erase(std::remove(active_tracks.begin(), active_tracks.end(), it->first), active_tracks.end());
//             // Check if the track is also in `past_active_tracks`
//             if (std::find(past_active_tracks.begin(), past_active_tracks.end(), it->first) != past_active_tracks.end()) {
//                 // If the track is found, remove it from `past_active_tracks` as well
//                 past_active_tracks.erase(std::remove(past_active_tracks.begin(), past_active_tracks.end(), it->first), past_active_tracks.end());
//             }
//             kf_map.erase(it->first);
//             // active_tracks_ids.erase(
//             //     std::remove(active_tracks_ids.begin(), active_tracks_ids.end(), it->first),
//             // active_tracks_ids.end());
//             active_tracks_ids.remove(it->first);
//             tracks_count--;
            
//             it = tracks_map.erase(it);
//         } else {
//             ++it;
//         }
//     }
//     EV << "CAR: " << mVehicleController->getId() << endl;
//     EV << "NUMBER OF TRACKS_IDS: " << active_tracks_ids.size() << endl;
//     EV << "NUMBER OF TRACKS: " << active_tracks.size() << endl;
//     EV << "NUMBER OF ACTIVE_TRACKS: " << active_tracks.size() << endl;
//     EV << "NUMBER OF PAST_ACTIVE_TRACKS: " << past_active_tracks.size() << endl;
...
//             if (reply.to_string()[0] == '{' && simTime() >= 61) {
...
//                     if (depth <= 5.5 && detections_map.find(role_name) == detections_map.end())
...
void CpService::checkTriggeringConditions(const SimTime& T_now)
{
    ...
    const SimTime T_GenCpmDcc = mDccRestriction ? genCpmDcc() : T_GenCpmMin;
    const SimTime T_elapsed = T_now - mLastCpmTimestamp;
    if (T_elapsed >= T_GenCpmDcc && T_now >= 1 && detections_map.size() != 0 ) { // active_tracks.size() != 0  && simTime() >= 61
        if (mFixedRate) {
            sendCpm(T_now);
        } else if (checkHeadingDelta() || checkPositionDelta() || checkSpeedDelta()) {
            sendCpm(T_now);
    ...
    omnetpp::cGroupFigure& det, omnetpp::cFigure::Point cpc)
{
    ...
    management.referencePosition.latitude = round(vdp.latitude(), microdegree) * Latitude_oneMicrodegreeNorth;
    // EV << "latitude: " << management.referencePosition.latitude << endl << "longitude: " << management.referencePosition.longitude << endl;
    management.referencePosition.longitude = round(vdp.longitude(), microdegree) * Longitude_oneMicrodegreeEast;
