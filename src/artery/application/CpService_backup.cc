...
void CpService::indicate(const vanetza::btp::DataIndication& ind, std::unique_ptr<vanetza::UpPacket> packet)  // when a CPM is received
{
    ...
    if (cpm && cpm->validate()) {
        ...
        for (int i = 0; i < msg->cpm.cpmParameters.perceivedObjectContainer->list.count; i++) {
            ...
            // // active_tracks_ids.push_back(object->objectID);
            ...
            if (mVehicleController->getVehicleId() == "car0") {
                std::string file_name = mVehicleController->getVehicleId() + "_cpm_objects.csv";
                cpms_file.open(file_name, std::ios_base::app);
                cpms_file << simTime() << "," << object->objectID << "," << cpc.x + ((double)object->xDistance.value / 100) << ","
                          << cpc.y + ((double)object->yDistance.value / 100) << endl;
                cpms_file.close();
            }
        }
        ...
        //     si->detectionArea.choice.vehicleSensor.vehicleSensorPropertyList.list.array[0]->horizontalOpeningAngleStart; EV << "horizontalOpeningAngleEnd: "
        //     << si->detectionArea.choice.vehicleSensor.vehicleSensorPropertyList.list.array[0]->horizontalOpeningAngleEnd;
        // }
...
void CpService::checkTriggeringConditions(const SimTime& T_now)
{
    ...
    if (T_elapsed >= T_GenCpmDcc && T_now >= 1 && tracks_map.size() != 0 && simTime() >= 1) {
        if (mFixedRate) {
            sendCpm(T_now);
...
vanetza::asn1::Cpm createCollectivePerceptionMessage(
    ...
    std::map<int, track_info> tm, omnetpp::cGroupFigure& det, omnetpp::cFigure::Point cpc)
{
    ...
    for (auto pair : tm) {
        ...
        auto key = pair.first;
        auto value = pair.second;
        EV << "perceived " << key << " to be sent!" << endl;
        // uss_setup desired_config = us[value.sensor];
...
// vanetza::asn1::Cpm createCollectivePerceptionMessage(
//     const VehicleDataProvider& vdp, uint16_t genDeltaTime, const VehicleController& vc, std::map<int, uss_setup> us, std::map<int, uss_value> dm,
//     std::map<int, track_info> tm, omnetpp::cGroupFigure& det, omnetpp::cFigure::Point cpc, std::ofstream cpmsf)
// {
//     vanetza::asn1::Cpm message;
...
//             std::string file_name = vc.getVehicleId() + "_cpm_objects.csv";
//             remove(file_name.c_str());
//             cpmsf.open(file_name , std::ios_base::app);
//             // cpms_file << "timestamp,track_id,x_track,y_track" << endl;
//             cpmsf << simTime() << "," << po->objectID << ","  << cpc.x + ((double)po->xDistance.value / 100) << "," << cpc.y + ((double)po->yDistance.value /
//             100); cpmsf.close();
//         }
//         ASN_SEQUENCE_ADD(cpm.cpmParameters.perceivedObjectContainer, po);
//     }
...
void CpService::updateEveryTimestamp()
{
    ...
    for (auto it = tracks_map.begin(); it != tracks_map.end();) {
        // Directly check timestamp without using active_tracks
        if (timestamp - it->second.timestamp > 1 || (timestamp - it->second.timestamp > 0.5 && detections_map[it->second.sensor].depth >= 5)) {
            // Remove from past_active_tracks if present
            EV << "Track " << it->first << " erased!" << endl;
            kf_map.erase(it->first);
            it = tracks_map.erase(it);  // Erase track from map
        } else {
            ++it;
        }
    }
    EV << "CAR: " << mVehicleController->getId() << endl;
    EV << "NUMBER OF TRACKS (tracks_map): " << tracks_map.size() << endl;
    for (auto pair : tracks_map)
        EV << "    track " << pair.first << endl;
    EV << "NUMBER OF KF: " << kf_map.size() << endl;
}
    ...
    do {
        ...
        if (reply.to_string()[0] == '{') {
            ...
            if (message_type == "USS_DATA") {
                ...
                //    << endl;
                // if (depth <= 5.5 && detections_map[role_name].timestamp != initial_timestamp)
                if (depth <= 5.5 && detections_map.find(role_name) == detections_map.end() && simTime() >= 1)  //
                    storeDetections(
                        role_name, initial_timestamp, detection_delta_time, std::round(depth * 1000) / 1000);  // storing the detections in the "detections_map"
            } else {
...
void CpService::storeDetections(int role_name, double timestamp, uint16_t detection_delta_time, double depth)
{
    ...
    EV << "timestamp:" << timestamp << endl << "role_name: " << role_name << endl << "     coord.x: " << coord.x << endl << "     coord.y: " << coord.y << endl;

    if (std::abs(detections_map[role_name].detection_coord.x - coord.x) < 0.01 && std::abs(detections_map[role_name].detection_coord.y - coord.y) < 0.01) {
        detections_map[role_name].repeat_count += 1;
        // EV << "if: " << detections_map[role_name].repeat_count << endl;
    } else {
        detections_map[role_name].repeat_count = 0;
        // EV << "else" << endl;
    }
    // EV << "entrou! < 20." << endl;
...
void CpService::determineObjects()
{
    ...
    for (auto pair : detections_map) {
        ...
        if (detections_map[key].repeat_count < 60) {
    ...
    EV << "CAR: " << mVehicleController->getId() << endl << "NUMBER OF OBJECTS: " << objects_map.size() << endl;
    ...
    for (auto pair : objects_map) {
        ...
        // auto arc = new cArcFigure();
        // arc->setBounds(cFigure::Rectangle(cur_pos_center.x + desired_config.sensor_pos.x, cur_pos_center.y + desired_config.sensor_pos.y, 2 * depth, 2 *
        // depth)); arc->setPosition(cur_pos_center + desired_config.sensor_pos, cFigure::ANCHOR_CENTER); arc->setStartAngle(desired_config.norm_detection_angle
        // - 30 * PI / 180 + PI); arc->setEndAngle(desired_config.norm_detection_angle + 30 * PI / 180 + PI); detections->addFigure(arc);
    }
}
...
//                 id = distrib(gen);
//             } while (tracks_map.find(id) != tracks_map.end());
//             // } while ((std::find(active_tracks_ids.begin(), active_tracks_ids.end(), id) != active_tracks_ids.end()));

//             // active_tracks_ids.push_back(id);  // objectIDs in use
//             auto key = dm.first;
...
//                     tracks_map[active_tracks[i]].speed = std::sqrt(std::pow(tracks_map[active_tracks[i]].detection_coord.x -
//                     objects_map[keys[min_cost_indices[i]]].detection_coord.x, 2)
...
//                 tracks_map[active_tracks[tracks_aux]].speed = std::sqrt(std::pow(tracks_map[active_tracks[tracks_aux]].detection_coord.x -
//                 objects_map[keys[pos_aux]].detection_coord.x, 2)
...
//         // EV << "cur time: " << cur_time << endl << "track time: " << tracks_map[at].timestamp << endl << (cur_time - tracks_map[at].timestamp <  0.05) <<
//         endl;
...
void CpService::multiObjectTracking()
{
    if (!objects_map.empty()) {
        ...
        if (tracks_map.empty()) {  // if there are no active tracks, all the objects will become their own track
            EV << "FIRST TRACKS TO BE CREATED!" << endl;
            for (const auto& dm : objects_map) {
                Identifier_t id = 0;
                // std::random_device rd;
                do {
                ...
                kf_map[id] = KalmanFilter(0.05, 0, 0, 0, 0.3, 0.3);
                objects_used.push_back(dm.first);
                // keys.erase(std::remove(keys.begin(), keys.end(), key), keys.end());
            }
        } else {
            EV << "NOT FIRST TRACKS TO BE CREATED!" << endl;
            std::vector<int> keys;
            for (auto pair : objects_map)
                keys.push_back(pair.first);
            ...
            for (size_t i = 0; i < cost_matrix.size(); ++i) {
                std::transform(
                    distance_matrix[i].begin(), distance_matrix[i].end(), direction_matrix[i].begin(), cost_matrix[i].begin(),
                    [distance_weight, direction_weight, distance_threshold](double d, double a) {
                        double cost = d * distance_weight + a * direction_weight;
                        return (d > distance_threshold) ? std::numeric_limits<double>::infinity() : cost;
                    });
            ...
            std::map<int, int> min_cost_indices;

            std::vector<int> track_ids;
            // track_ids.reserve(tracks_map.size());
            EV << "TRACK_IDS:" << endl;
            for (const auto& [k, v] : tracks_map) {
                EV << "    " << k << endl;
                track_ids.push_back(k);
            }
            min_cost_indices = optimal_assignment(cost_matrix, track_ids);

            for (auto pair : min_cost_indices) {
                auto key = pair.first;
                auto value = pair.second;
                EV << "MIN_COST_INDICES:" << endl << "    KEY: " << key << endl << "    VALUE: " << value << endl;
                if (value != -1) {
                    omnetpp::cFigure::Point pq = objects_map[value].detection_coord - tracks_map[key].detection_coord;
                    double magPq = std::sqrt(std::pow(pq.x, 2) + std::pow(pq.y, 2));
                    double angl = std::acos(pq.x / magPq);
                    if (pq.y > 0)
                        angl = angl + PI;
                    tracks_map[key].direction = angl;
                    if (objects_map[value].timestamp != tracks_map[key].timestamp) {
                        tracks_map[key].speed = std::sqrt(
                                                    std::pow(tracks_map[key].detection_coord.x - objects_map[value].detection_coord.x, 2) +
                                                    std::pow(tracks_map[key].detection_coord.y - objects_map[value].detection_coord.y, 2)) /
                                                (objects_map[value].timestamp - tracks_map[key].timestamp);
                    } else {
                        tracks_map[key].speed = 0;
                    }


                    tracks_map[key].detection_coord = objects_map[value].detection_coord;
                    tracks_map[key].timestamp = objects_map[value].timestamp;
                    tracks_map[key].sensor = keys[value];
                    objects_used.push_back(keys[value]);
                    auto prediction = kf_map[key].predict();

                    kf_map[key].update({tracks_map[key].detection_coord.x, tracks_map[key].detection_coord.y});
                    auto updated = kf_map[key].getPosition();
                    tracks_map[key].detection_coord = omnetpp::cFigure::Point(updated(0), updated(1));
                    auto coord = tracks_map[key].detection_coord;
...
std::pair<std::vector<std::vector<double>>, std::vector<std::vector<double>>> CpService::generate_cost_matrices()
{
    ...
    size_t i = 0;
    for (const auto& track_pair : tracks_map) {  // iterate directly over tracks_map
        const auto& track_id = track_pair.first;
        const auto& pred_track_pos = track_pair.second;

        int j = 0;
        for (const auto& obj_pair : objects_map) {
            const auto& detection_pos = obj_pair.second.detection_coord;
            ...
            // Use past_tracks_map if this track_id exists there
            auto past_it = past_tracks_map.find(track_id);
            if (past_it != past_tracks_map.end()) {
                const auto& past_pos = past_it->second.detection_coord;

                double delta_x = past_pos.x - pred_track_pos.detection_coord.x;
                double delta_y = past_pos.y - pred_track_pos.detection_coord.y;
...
std::map<int, int> CpService::optimal_assignment(
    const std::vector<std::vector<double>>& cost_matrix, const std::vector<int>& track_ids)  // Pass track IDs so we can map
{
    ...
    // Augment matrix with dummy column for unassigned
    std::vector<std::vector<double>> augmented_matrix = cost_matrix;
    for (auto& row : augmented_matrix) {
        row.push_back(UNASSIGNED_COST);
    }

    // Prepare vector to generate permutations of detection assignments to tracks
    // Length = num_tracks, each element is an assignment index (0 to num_detections or unassigned)
    std::vector<int> indices(num_tracks);

    // Initialize with 0,1,2,...,num_tracks-1 for permutations
    // This works assuming num_detections >= num_tracks
    // If num_detections < num_tracks, problem is underspecified (need to handle differently)
    ...
    do {
        ...
        for (size_t i = 0; i < num_tracks; ++i) {
            ...
            // Assignment index must be in range of detections + unassigned dummy (num_detections)
    ...
    std::map<int, int> assignment_map;
    for (size_t i = 0; i < track_ids.size(); ++i) {
        int track_id = track_ids[i];

        if (best_assignment[i] >= num_detections) {
            assignment_map[track_id] = -1;  // Unassigned
        } else {
            assignment_map[track_id] = best_assignment[i];
        }
    }
